...
def mutex_watchdog_thread_function():
    ...
    while True:
        if lock_try_time != 0 and time.time()-lock_try_time > 3.0:
            ...
            print("[openwebrx-mutex-watchdog] Mutex unlock timeout. Locker: \"" + str(
                clients_mutex_locker) + "\" Now unlocking...")
            clients_mutex.release()
        time.sleep(0.5)
...
class WebRXHandler(BaseHTTPRequestHandler):
    def send_302(self,what):
        ...
        self.end_headers()
        self.wfile.write("<html><body><h1>Object moved</h1>Please <a href=\"/{0}\">click here</a> to continue.</body></html>".format(what).encode())
    ...
    def do_GET(self):
        ...
        try:
            ...
            if self.path[:4]=="/ws/":
                ...
                anyStringsPresentInUserAgent=lambda a: reduce(lambda x,y:x or y, map(lambda b:headers['user-agent'].count(b), a), False)
                if extension == "wrx" and ( (not anyStringsPresentInUserAgent(("Chrome","Firefox","Googlebot","iPhone","iPad","iPod"))) if 'user-agent' in headers.keys() else True ) and (not request_param.count("unsupported")):
