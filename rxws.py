...
def handshake(myself):
    ...
    my_header_keys = [key.lower() for key, value in myself.headers.items()]
    ...
    ws_key_toreturn=base64.b64encode(hashlib.sha1((ws_key+"258EAFA5-E914-47DA-95CA-C5AB0DC85B11").encode()).digest()).decode()
...
def get_header(size):
    ...
    if(size>125):
        ...
        extended_size=bytes([(size>>8)&0xff, size&0xff]) #Okay, it uses reverse byte order (little-endian) compared to anything else sent on TCP
    else:
        ws_second_byte=size
        #256 bytes binary message in a single unmasked frame | 0x82 0x7E 0x0100 [256 bytes of binary data]
        extended_size=bytes()
    return bytes([ws_first_byte, ws_second_byte])+extended_size

def code_payload(data, key=bytes()):
    # both encode or decode
    if len(key) == 0:
        key = bytes([61, 84, 35, 6])
    encoded = [v ^ key[i % 4] for i, v in enumerate(data)]
...
def recv(myself, blocking=False, debug=False):
    ...
    #always block here, otherwise the framing gets corrupted.
    #would need a state machine and backbuffering to be able to run non-blocking
    data+=readsock(myself,length,True)
    ...
    if fin and is_text_frame and has_one_byte_length:
        if masked:
            return code_payload(data[6:], data[2:6]).decode()
        else:
